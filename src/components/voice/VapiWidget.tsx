        ...
        <div className="max-w-5xl mx-auto px-4 flex flex-col overflow-hidden pb-20">
            <div className="text-center mb-8">
                <h1 className="text-3xl font-bold font-mono">
                    ...
                    <span className="text-primary uppercase">AI Assistant</span>
                </h1>
                <p className="text-muted-foreground mt-2">
                    Have a conversation with your AI assistant using voice commands.
                </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
                    <div className={`absolute inset-0 ${ isSpeaking ? "opacity-30" : "opacity-0"} transition-opacity duration-300 `}>
                        ...
                        {[...Array(5)].map((_, i) => (
                            <div
                                key={i} className={`mx-1 h-16 w-1 bg-primary rounded-full" ${isSpeaking ? "animate-sound-wave" : ""}`}
                    ...
                    <div className="relative size-32 mb-4">
                        ...
                        <div className="relative w-full h-full rounded-full bg-card flex items-center justify-center border border-border overflow-hidden">
                            ...
                            <Image src="/logo.png" alt="AI Assistant" width={80} height={80} className="w-20 h-20 object-contain" />
                        </div>
                    </div>
                    <h2 className="text-xl font-bold text-foreground">AivaDent AI</h2>
                    ...
                    <div className={`mt-4 flex items-center gap-2 px-3 py-1 rounded-full bg-card border border-border ${isSpeaking ? "border-primary" : ""}`} >
                        ...
                        <span className="text-sm text-muted-foreground">
                            {isSpeaking ? "Speaking..." : callActive ? "In Call" : callEnded ? "Call Ended..." : "Waiting..."}
                        </span>
                    </div>
                ...
                <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
                    ...
                    <div className="relative size-32 mb-4">
                            <Image src={user?.imageUrl!} alt="" width={128} height={128} className="size-full object-cover rounded-full" />
                        </div>
                    ...
                    <div className="space-y-3">
                        {messages.map((msg, index) => (
                            <div key={index} className="message-item animate-in fade-in duration-300">
                                <div className="font-semibold text-xs text-muted-foreground mb-1">
                                    {msg.role ==="assistant" ? "AivaDent AI" : "You"}:
                                </div>
                        ...
                        {callEnded && (
                            <div className="message-item animate-in fade-in duration-300">
                                 ...
                                 <p className="text-foreground">
                                    Call has ended. Thank you for using AivaDent AI!
                                </p>
                            </div>
                        )}
                    </div>
            ...
            <div className="w-full flex justify-center gap-4">
                <Button className={`w-44 text-xl rounded-3xl ${callActive ? "bg-destructive hover:bg-destructive/90" : callEnded ? "bg-red-500 hover:bg-red-700" : "bg-primary hover:bg-primary/90"} text-white relative`} onClick={toggleCall} disabled={connecting || callEnded} >
                    {connecting && (
